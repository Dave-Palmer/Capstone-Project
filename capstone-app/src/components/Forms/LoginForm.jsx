...
export default function LoginForm() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            let response = await axios.post('http://localhost:8009/users/user/login', { email: email, password: password })
            loggedInUser = response.data.data
            handleUpdateUser(loggedInUser)
            loggedInUser.admin ? navigate('/home') : navigate('/dashboard')
        }
        catch (error) { setErrorMessage(error.response.data.result) }
    };
